...
func (c Catalog) Find(id string) (Model, error) {
	...
	return Model{}, fmt.Errorf("OpenRouter model %q is not in the current catalog; run clother config openrouter", id)
}
...
func Load(ctx context.Context, baseURL, cacheDir string, refresh bool) (Catalog, error) {
	...
	if err != nil {
		return Catalog{}, fmt.Errorf("invalid OpenRouter model endpoint")
	}
	client := &http.Client{Timeout: 10 * time.Second}
	resp, err := client.Do(req)
	if err != nil {
		return Catalog{}, fmt.Errorf("could not fetch OpenRouter models: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return Catalog{}, fmt.Errorf("OpenRouter model catalog returned HTTP %d", resp.StatusCode)
	}
	var catalog Catalog
	if err := json.NewDecoder(io.LimitReader(resp.Body, 16<<20)).Decode(&catalog); err != nil || len(catalog.Models) == 0 {
		return Catalog{}, fmt.Errorf("invalid or empty OpenRouter model catalog")
	}
